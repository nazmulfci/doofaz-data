      ...
      <form @submit.prevent="create()">
        ...
        <div class="col-lg-8 px-lg-5 col-sm-8 offset-sm-2 col-12 supplier-border comm-form-box-back-color">
          ...
          <div class="form-group">
              ...
              <select v-model="form.ownerTypeId" @change.prevent="changeCompanyType()" name="ownerTypeId" class="form-control" :class="{ 'is-invalid': form.errors.has('ownerTypeId') }">
                  <option value=""> ------ </option>
           ...
           <div class="form-group" v-if="subHeadStatus">
             <label> Owner Name <i class="fa fa-star text-danger text-10 cursor-pointer" title="Mandatory Field" data-placement="top" data-toggle="tooltip" data-original-title="Mandatory Field"></i></label>
             ...
             <has-error :form="form" field="subHead"></has-error>
               ...
               <div class="form-group">
                <label> Capital  <i class="fa fa-star text-danger text-10 cursor-pointer" title="Mandatory Field" data-placement="top" data-toggle="tooltip" data-original-title="Mandatory Field"></i></label>
...
import Multiselect from "vue-multiselect";
   export default {
       ...
       methods: {
        toatlShareCalculation(){
            let useShare = parseFloat(this.form.shareUse);
            let totalshare = parseFloat(this.form.totalShare);
            let availableShare = totalshare-useShare;
            
            let shareAmount = parseFloat(this.form.authorizeShare);
            let valueAmount = parseFloat(this.form.perShareValue);

            let useableAmount = availableShare-shareAmount;
            //alert(useableAmount);
   ...
   axios.get('/condition').then(res =>{
      ...
      this.form.perShareValue = res.data.authInfo.perShareValue;
      this.form.shareUse = res.data.authInfo.shareUse;
      this.form.totalShare = res.data.authInfo.authorizeShare;
      ...
      if(res.data.authInfo.shopLicenceTypeId>2){
        ...
        this.limitedCompanyInformation=false;
         ...
         clearForm(){
           ...
           create() {
               this.loading = true;
               this.form.post('/shareCapitalEntry').then(res => {
                  this.loading = false;
                     Toast.fire({
                         icon: 'success',
         ...
         Toast.fire({
           ...
           changeCompanyType(){
              axios.get('getCompanyOwnerByType/'+this.form.ownerTypeId).then((response) => {
                       this.subHeadStatus = true;
                       this.subHeadOfAccountLists = response.data.data;
                     });
           },
