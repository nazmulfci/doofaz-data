  ...
  <span>
    <div class="card main-card  element-block-example">
      <div class="card-header" style="background-color: rgb(1, 176, 241); border: 1px solid rgba(0, 0, 0, 0.05);">
        <h3 style="color:black">Opening Product Report</h3>
      </div>
           ...
           <thead>
            ...
            <tbody>
              <tr v-for="(purchaseProductReport, index) in purchaseProductReports.data">
                ...
                <td>
                  <router-link :to="`/openingproduct@reportList${purchaseProductReport.productId}`" class="mr-2 btn-hover-shine btn btn-shadow btn-primary"><i class=" fa fa-eye pr-2"></i>View Details</router-link>
                </td>
              </tr>
            </tbody>
        </table>
...
<script>
  export default {
      ...
      methods:{
          productSupplierList(){
            axios.get('/openingProductReport').then(res =>{
                this.purchaseProductReports = res.data.purchaseProductReports
            })
          },
          getPaginateList(page = 1){
            axios.get('/openingProductReport?page=' + page).then(response => {
