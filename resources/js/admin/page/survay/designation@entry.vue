      ...
      <div class="supplier-entry py-1 px-1">
        <div class="card-body table-responsive bg-white">
          <table class="table table-hover table-bordered mb-0">
            ...
            <tbody>
              <tr v-for="(list, index) in DesignationLists">
                <td>{{ ++index }}</td>
...
export default {
  ...
  methods: {
    addDesignation() {
      this.form.post("designation/entry").then((response) => {
        Toast.fire({
          icon: "success",
    ...
    DesignationShow() {
      axios.get("get/designation/list").then((response) => {
        this.DesignationLists = response.data.data;
      });
    },

    deactivateStatus(id) {
      axios.get("designation/deactivate/status/" + id).then((response) => {
        this.DesignationShow();
        Toast.fire({
          icon: "success",
    ...
    activateStatus(id) {
      axios.get("designation/activate/status/" + id).then((response) => {
        this.DesignationShow();
        Toast.fire({
          icon: "success",
