<template id="">
          ...
          >
              ...
              <div class="form-group mt-2">
                <label> এরিয়া 
                  ...
                  <label v-for="areaList,index in areaLists" style="padding:0px 5px;">
                      <input type="checkbox" v-model="form.areaId" :value="areaList.id">
                       <span>{{ areaList.areaName }}</span>
                  <!--   <span v-for="alist in aponLists" v-if="areaList.id == alist.areaId">
                      <span>{{ alist.area.areaName }}</span>
                    </span> -->
                  </label>
  ...
  <!-- -----------------------------   // card footer  --> 
      ...
      <div class="card mt-2 lists" v-for="employeeName in employeeNames">
          <div class="">
            <h4>
               {{ employeeName.userName }}
               <router-link :to="`/edit@area@assign${employeeName.id}`" class="btn btn-sm btn-info">Edit</router-link>
            </h4>

            <ol>
              <li v-for="(area, index) in viewAssignLists" v-if="employeeName.id==area.employeeId">
                 <span>{{ area.area.areaName }}</span>
                 <!-- <span v-if="area.status==1" class="active_btn">active</span> -->
              </li>
            </ol>
        </div>
      </div>
  ...
  export default {
    ...
    viewAssignList() {
      ...
      this.loading=true;
        ...
        this.form.post("/areaAssignEntry").then((res) => {
       ...
       shopMenuPermissionShow(){
          axios.get('/shopPermissionAreaShows/'+this.form.employeeId).then(res =>{
