...
<button class="btn btn-primary"  @click.prevent="back(shopInformation.id)">Back</button>
...
export default {
  ...
  back(id){    
      axios.post(`informationneedBack/${id}`,{reason:this.reason})     
        .then((response) => {
            Toast.fire({
                icon: 'success',
    ...
    view() {
      let url = "informationNeedStatusup";
      axios.get(url)
