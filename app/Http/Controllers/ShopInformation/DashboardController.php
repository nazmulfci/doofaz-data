...
class DashboardController extends Controller
{
    ...
    public function searchInfo(Request $request){
        ...
        // dd($requestData);
            ...
            if($request->shopOfficeName){
                $name = explode('(',$request->shopOfficeName);
                $name = rtrim($name[0]);
                $datas = $datas->where('shopOfficeName','LIKE',"%$name%");
            }
            
            if($request->mobileNo){
                $datas = $datas->where(function ($query) use ($request) {
                    $query->where('mobileNo',$request->mobileNo)
                        ->orWhere('whatsapp',$request->mobileNo)
                        ->orWhere('contactNo',$request->mobileNo)
                        ->orWhere('owner_phone',$request->mobileNo);
                });
            }
